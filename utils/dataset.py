...
import torch
from utils.cfg import CLASSES, DATASET_PATH, NUM_BBOX
...
class Dataset(data.Dataset):
    def __init__(self, is_train = True, is_aug = True) -> None:
        ...
        """
        super().__init__()
        self.root = os.getcwd() + '/yolo_helmet_train/anno/'
...
# 瓜分数据为 训练数据集 和valid
def split_train_valid(valid_per):
    img_path = os.getcwd() + "/yolo_helmet_train/anno/images/"
    label_path = os.getcwd() + "/yolo_helmet_train/anno/labels/"
    size = len(os.listdir(img_path))
    labels_list = os.listdir(label_path)
    train_size = int(size * (1 - valid_per))
    ...
    if not os.path.exists(label_path + 'valid'):
        os.mkdir(label_path + 'valid')
        
    for i, file in enumerate(os.listdir(img_path)):
        if i < train_size:
            shutil.copyfile(img_path + file, img_path + "train/" + file)
            shutil.copyfile(label_path + file.split('.')[0] + ".txt", label_path + "train/" + file.split('.')[0] + ".txt")
        else :
            shutil.copyfile(img_path + file, img_path + "valid/" + file)
            shutil.copyfile(label_path + file.split('.')[0] + ".txt", label_path + "valid/" + file.split('.')[0] + ".txt")
...
def show_labels_img(imgname):
	# """imgname是输入图像的名称，无下标"""
    print(DATASET_PATH + "images/" + imgname + ".jpg")
